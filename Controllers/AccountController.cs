...
public IActionResult SignIn([FromBody] LoginModel model)
{
    if (ModelState.IsValid)
    {
        try
        {
            ...
            var result = _client.PostAsync("https://joke.qwave.cloud/api/authentication/login", new StringContent(Newtonsoft.Json.JsonConvert.SerializeObject(model), System.Text.Encoding.UTF8, "application/json")).Result;
